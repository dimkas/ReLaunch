...
public class ReLaunch extends Activity{
    ...
    final static private String APP_KEY = "peevrqmgpqh2usm";
    final static private String APP_SECRET = "h282lvxdpd9sbn5";
                    ...
                    if (useFaces) {  // прочитанные/новые
                        ...
                        if (app.history.containsKey(fname)) { // смотрим историю по файлу
                            ...
                            if (baseHistory == app.READING) { // если в базе - читается
                                ...
                                holder.tvHolder.setBackgroundColor(getResources().getColor(R.color.file_reading_bg));
                                ...
                                holder.tvHolder.setBackgroundColor(getResources().getColor(R.color.file_unknown_bg));
                            }
                        } else { // если файл определен как новый
                            color_txt = getResources().getColor(R.color.file_new_fg);
                            holder.tvHolder.setBackgroundColor(getResources().getColor(R.color.file_new_bg));
            ...
            public boolean onSingleTapConfirmed(MotionEvent e) {
                ...
                if (item.get("type").equals("dir")) {
                    ...
                    if(tempName.startsWith("Dropbox| ")){
                        fname = loadFileDB(tempName.substring("Dropbox| ".length() + 1), item.get("name"));
                    }else{
                        fname = tempName;
                    }
...
//		AdapterContextMenuInfo info = (AdapterContextMenuInfo) item.getMenuInfo();
        ...
        switch (itemId) {
            case CNTXT_MENU_SET_STARTDIR:
                ...
                drawDirectory(fullName, -1); //можно пробовать
                break;
            case CNTXT_MENU_ADD_STARTDIR:
                app.addStartDir(fullName); // можно пробовать
                break;
            case CNTXT_MENU_ADD:
                if (type.equals("file"))   // можно пробовать
                    app.addToList("favorites", dname, fname, false);
                else
                    app.addToList("favorites", fullName, app.DIR_TAG, false);
                break;
            case CNTXT_MENU_MARK_READING:
                app.history.put(fullName, app.READING);   // можно пробовать
                app.saveList("history");
                reDraw();
                break;
            case CNTXT_MENU_MARK_FINISHED:
                app.history.put(fullName, app.FINISHED);  // можно пробовать
                app.saveList("history");
                reDraw();
                break;
            case CNTXT_MENU_MARK_FORGET:
                app.history.remove(fullName);     // можно пробовать
                app.saveList("history");
                reDraw();
                break;
            case CNTXT_MENU_OPENWITH: {       // можно пробовать
            ...
            case CNTXT_MENU_INTENT: { // можно пробовать
            ...
            case CNTXT_MENU_COPY_FILE:// можно пробовать
                fileOpFile = fname;
                fileOpDir = dname;
                fileOp = CNTXT_MENU_COPY_FILE;
                break;

            case CNTXT_MENU_MOVE_FILE:// можно пробовать
                fileOpFile = fname;
                fileOpDir = dname;
                fileOp = CNTXT_MENU_MOVE_FILE;
                break;

            case CNTXT_MENU_COPY_DIR:// можно пробовать
                fileOpFile = fname;
                fileOpDir = dname;
                fileOp = CNTXT_MENU_COPY_DIR;
                break;

            case CNTXT_MENU_PASTE:// можно пробовать
                String src;
                ...
                if(!dropSrc && dropDst){
                    if (fileOp == CNTXT_MENU_COPY_FILE || fileOp == CNTXT_MENU_COPY_DIR){
                        ...
                        retCode = DBupload(src, dname);
                        app.removeFile(fileOpDir, fileOpFile);
                    }
                }
                ...
                if (retCode) {
                    ...
                    builder.setMessage(getResources().getString(R.string.jv_relaunch_paste_fail_text)+ " " + fileOpFile);
            ...
            case CNTXT_MENU_TAGS_RENAME: { // можно пробовать
            ...
            case CNTXT_MENU_RENAME: { // можно пробовать
                final Context mThis = this;
            ...
            case CNTXT_MENU_CREATE_DIR: { // можно пробовать
                final Context mThis = this;
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
                final EditText input = new EditText(this);
                builder.setView(input);
            ...
            case CNTXT_MENU_SWITCH_TITLES: // можно пробовать
            ...
            case CNTXT_MENU_SHOW_BOOKINFO:// можно пробовать
                String temp = fullName;
                if(temp.startsWith("Dropbox| ")) {
    ...
    private void screenHome() {
        ...
        for (Integer i = 0; i < startDir.length; i++) {
